...
// module that talks to the Colyseus server (../../Ice-skate-backend's
// RinkRoom). Exactly like systems/bloxity.js, every path through here is
...
//
// The server is a dumb relay: we send our transform as a `move` message, it
// fans every player's state out through `room.state.players`. We read that
// map once per frame in step() rather than wiring per-field schema
// callbacks, so there is no dependency on the exact @colyseus/schema
// callback API.
...
// id -> live remote body. Mutated in place; step() and getLeaderboard()
...
// subscriber never re-renders per frame.
...
// "everyone online right now" (server RinkRoom.ts's refreshLeaderboard()),
...
// build the real character. `avatarState` already holds the SDK's equipped
// ids + clamped proportions; a guest's `equipped` is {} which still builds
// the bare base rig, exactly like the local PlayerAvatar.
...
// Keep only string-valued cosmetic ids, each length-capped.
...
// --- Stats sync ---------------------------------------------------------
// Our own live speed/rebirth/wins (store/useGameStore.js), pushed to the
// room so an in-world leaderboard can rank currently-connected players.
// useGameStore is zustand's plain store object — .getState()/.subscribe()
// are framework-free (no React import here, same constraint as the rest of
...
// scheduleAvatarResend, just a longer window (STATS_RESEND_DEBOUNCE_MS): an
// actively-grinding AFK player can gain Speed many times a second, and a
// per-gain packet would be needless chatter for a leaderboard that only
// needs a roughly-current rank.
...
// Last speed/rebirth/wins we scheduled a resend for — useGameStore.subscribe
...
// The durable half of store/useGameStore.js — speed/rebirth/wins plus owned
// and equipped hex pads/auras, owned wins-gated targets, and moveSpeed —
// pushed to a signed-in player's own document in the server's Mongo
// `players` collection (server src/db.ts, RinkRoom.ts's `saveProgress`). A
// guest has no stable id (systems/bloxity.js getStableUserId(), which this
// gates on) and this simply never sends for one — same "nowhere durable to
// live" stance as the rest of the SDK integration.
function progressPayload() {
  ...
  return {
    ...
    moveSpeedLevelBonus: s.moveSpeedLevelBonus,
  }
}
...
// on whether this session is allowed to persist (RinkRoom.ts's `userIds`
...
// this hits Mongo on the other end, not just an in-memory schema field, and
// a save a few seconds behind is harmless since teardown() flushes once
// more on the way out.
...
// via login/logout without a page reload), so without this, RinkRoom.ts's
  ...
  try {
    ...
    const joined = await withTimeout(
      ...
      client.joinOrCreate(ROOM_NAME, {
        ...
        // Empty string for a guest — RinkRoom.ts's onJoin treats a falsy
...
function attachRoom(joined) {
  ...
  // The saved doc for our own Bloxity user id (RinkRoom.ts's onJoin ->
  ...
  // The merged all-time + online leaderboard (server RinkRoom.ts's
  ...
  // Re-send our avatar on every (re)attach — the join options already
  // carried it, but a fresh joinOrCreate after a drop needs it re-stated on
  // the new session.
  sendAvatarNow()
  // Same reasoning for stats: a fresh session starts every field at its
  // schema default (0), so a rejoin needs its current speed/rebirth/wins
...
export function init() {
  ...
  // Our own speed/rebirth/wins -> the room, debounced (see scheduleStatsResend).
...
let lastSendAt = 0
const lastSent = { x: 0, y: 0, z: 0, yaw: 0, moveBlend: 0 }

// Called every frame from GameLoop. Throttles itself: MOVE_SEND_HZ while the
// transform is changing, once per MOVE_IDLE_MS at rest so a late joiner
// still sees us standing where we are.
export function reportLocal() {
  if (!room) return

  const p = player.position
  const yaw = player.facing
  // 0..1 eased gait factor for a remote's skate animation blend — our
  // current speed against our own current max walk speed (player.moveSpeed,
  // set every frame by systems/playerMovement.js from the equipped skate).
  const moveBlend = Math.min(1, Math.hypot(player.velocity.x, player.velocity.z) / (player.moveSpeed || 1))
  ...
  const changed =
    ...
    Math.abs(moveBlend - lastSent.moveBlend) > 0.05
  ...
  lastSent.z = p.z
  lastSent.yaw = yaw
  lastSent.moveBlend = moveBlend

  try {
    room.send('move', { x: p.x, y: p.y, z: p.z, yaw, moveBlend })
  } catch {
...
function ingestRemote(id, s, now) {
  ...
  if (!e) {
    // MAX_REMOTE_BODIES bounds the avatar-rig draw-call cost;
    // getLeaderboard() below reuses this same tracked set rather than a
    // separate channel, so with more than that many other players connected
    // the leaderboard only ranks whichever ones got tracked first, not
    // necessarily the true top scorers room-wide.
    if (remotePlayers.size >= MAX_REMOTE_BODIES) return
    const parsed = parseAvatar(s.avatar)
    e = {
      x: s.x, y: s.y, z: s.z, yaw: s.yaw, moveBlend: s.moveBlend,
      rx: s.x, ry: s.y, rz: s.z, ryaw: s.yaw, rmoveBlend: s.moveBlend,
      username: s.username || 'Player',
      // Bloxity avatar for a future RemotePlayers.jsx. `avatarRaw` is the
      // last schema string we built from; `avatarRev` bumps whenever it
      // changes so a rig component knows to rebuild.
      avatarRaw: s.avatar || '',
...
export function step(dt) {
  ...
  // riding out) into the HUD as "Reconnecting…", and back to "online" once
  // it recovers. If it never recovers, room.onLeave -> handleLeave takes over.
  ...
  for (const [id, e] of remotePlayers) {
    ...
    e.rmoveBlend += (e.moveBlend - e.rmoveBlend) * lerp
...
// --- Leaderboard --------------------------------------------------------
// Top `limit` players by `stat` ('speed' | 'rebirth' | 'wins' — any
...
// other row comes from globalLeaderboard (server RinkRoom.ts's
