...
// Shared chrome for every left-center HUD popup (Rebirth, Aura, Shop): a
...
function HudModal({ title, onClose, isTouch, children }) {
  return (
    <div
      ...
      // regardless of what's under the cursor. Stop it here so scrolling a
      // popup's content doesn't also zoom the camera behind it.
  ...
  return (
    <HudModal title="Rebirth" onClose={onClose} isTouch={isTouch}>
      ...
      <div className={`flex w-full items-stretch justify-center ${isTouch ? 'mb-3 gap-2' : 'mb-5 mt-2 gap-3'}`}>
        ...
        <span className={`self-center font-black text-slate-100 ${isTouch ? 'text-sm' : 'text-xl'}`}>or</span>
        ...
        <div className="relative flex-1 self-center">
          <button
            ...
            style={{
              background: 'linear-gradient(90deg, #ff3b3b, #ff9d00, #ffee00, #4dff4d, #33d1ff, #6f6fff, #c96fff)',
...
// through three states: not owned (buy with Wins) -> owned, not equipped
// (Equip) -> owned and equipped (Unequip next to a non-interactive
// "Equipped" pill).
...
// Coin glyph for a shop card's price pill — a stroked hexagon, reused at
// any size.
...
// fulfil SDK.bux.requestPurchase (server-to-server webhook), and hidden
...
function ShopItemCard({ item, isTouch }) {
  ...
  return (
    ...
    >
      <div className={`text-center font-black text-white ${isTouch ? 'py-1.5 text-xs' : 'py-2.5 text-lg'}`} style={textOutline}>
        {item.name}
      </div>
...
// Opened by holding E against a wins-gated AFK target before it's owned.
// Not reachable in this project (data/afk.js's AFK_TARGET_CONFIG is
// empty), kept wired for the day targets exist.
function TargetPurchaseWindow({ targetId, onClose, isTouch }) {
  ...
  return (
    <HudModal title={name} onClose={onClose} isTouch={isTouch}>
      <div className={`text-center font-bold text-white ${isTouch ? 'text-sm' : 'text-xl'}`} style={textOutline}>
...
function LeftCenterControls() {
  ...
  const auraWindow =
    showAuraWindow && createPortal(<AuraWindow isTouch={isTouch} onClose={() => setShowAuraWindow(false)} />, document.body)

  const shopWindow =
    showShopWindow && createPortal(<ShopWindow isTouch={isTouch} onClose={() => setShowShopWindow(false)} />, document.body)

  const targetPurchaseWindow =
    purchaseTargetId &&
    createPortal(
      <TargetPurchaseWindow targetId={purchaseTargetId} isTouch={isTouch} onClose={() => setPurchaseTargetId(null)} />,
      document.body,
    )
...
// Circumference of the hold-progress ring's r=15 circle.
...
// afk/hexPad/merchant below all render one of these. Written imperatively
// (setText/setHoldProgress, via a ref) on a ~10Hz poll, so this stays a DOM
// sibling that never re-renders per frame.
const InteractPrompt = forwardRef(function InteractPrompt(_props, ref) {
  ...
  return (
    <div
      ...
      style={{ display: 'none' }}
    >
      <span ref={keyWrapRef} className="relative flex h-9 w-9 shrink-0 items-center justify-center transition-colors duration-300">
        <svg viewBox="0 0 36 36" className="absolute inset-0 h-9 w-9 -rotate-90">
          <circle cx="18" cy="18" r={HOLD_RING_RADIUS} fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2.5" />
          <circle
            ref={ringRef}
            cx="18"
            cy="18"
...
// DOM siblings of the canvas, never drei <Html>. Must not re-render per
// frame: the proximity prompts are written through InteractPrompt's setText
// ref on a ~10Hz interval that reads the singletons directly. React state
// here is only for panels and portal-driven settings, which change at
// human speed.
export default function Hud() {
  ...
  // Dead in the PVP zone — this project has no PVP zone, so playerHealth.dead
  // never goes true, but the prompt stays wired.
  useEffect(() => {
    const id = setInterval(() => {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 p-4 font-mono text-xs leading-5 text-slate-200">
      {/* First child: the touch look-zone paints beneath the interactive HUD
         panels so their taps still land, while its own Fire/Jump/E buttons
         sit at z-40 above everything. */}
      <TouchControls />
      ...
      {/* Full-screen "rotate to landscape" gate for touch sessions. Last
         child + z-100 so it covers the touch controls while it is up. */}
      <RotatePrompt />
    </div>
  )
}
