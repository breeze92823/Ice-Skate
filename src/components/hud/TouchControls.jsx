...
// only React state here is the transient joystick thumb position and the
...
//                               or TAP to move the aim crosshair there
...
// one-finger press that never travels past TAP_MOVE_THRESHOLD is instead
// read as a tap and moves the aim crosshair there (sticky until the next
// such tap).
function LookZone() {
...
// lower wedge, so it never has to be found by feel.
function MoveStick() {
...
// Drawn at the sticky tap-to-aim point, starting at screen centre — a
// re-render on each tap, not a per-frame follow.
function Crosshair() {
...
export default function TouchControls() {
  ...
  // Backgrounding the tab mid-gesture must not leave the avatar walking or
  // the Fire button stuck on. systems/input.js clears the singleton on
  // window blur too; this keeps our own visuals honest.
  useEffect(() => {
    if (!on) return
    const stop = () => {
      setTouchMove(0, 0)
      ...
      >
        <div className="flex flex-col gap-3">
          <ActionButton
            ...
            sub="USE"
          />
          <ActionButton onPress={pressTouchJump} size={smallSize} label="JUMP" />
        </div>
        <ActionButton
